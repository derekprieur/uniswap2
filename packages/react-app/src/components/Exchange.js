    ...
    return (
        <div className='flex flex-col w-full items-center'>
            ...
            <div className='mb-8 w-[100%]'>
                <AmountOut fromToken={fromToken} toToken={toToken} amoutIn={fromValueBigNumber} pairContract={pairAddress} currencyValue={toToken} onSelect={onToTokenChange} currencies={counterPartTokens} />
                <Balance tokenBalance={toTokenBalance} />
            </div>
            {approvedNeeded && !isSwapping ? (
                <button className={`${styles.actionButton} ${canApprove ? 'bg-site-pink text-white' : 'bg-site-dim2 text-site-dim2'}`} disabled={!canApprove} onClick={onApproveRequested}>
                    {isApproving ? 'Approving...' : 'Approve'}
                </button>
            ) : (
                <button className={`${styles.actionButton} ${canSwap ? 'bg-site-pink text-white' : 'bg-site-dim2 text-site-dim2'}`} disabled={!canSwap} onClick={onSwapRequested}>
